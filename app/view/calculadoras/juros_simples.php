...
<html>
    ...
    <div class="container-form">
        <h3 class="title-form">Calculadora de Juros Simples</h3>
    ...
    <script type="text/javascript">
		...
		window.onload = function(){
			...
			calcular.onclick = function(){
				...
				for (var i = 1; i <= tempo.value; i++) {

                    let TotalCalc = capitalFloat*(juros.value/100);

                    somaTotal += TotalCalc;

                    let total2 = somaTotal+capitalFloat;
					TotalCalc = formatter.format(somaTotal+capitalFloat);

					montante += "<tr><td>"+(i)+"</td><td style='width: 50%;'>"+(TotalCalc) + "</td></tr>";

					if (i == tempo.value) {
						montante += "<tr><td>Rendimento Líquido</td><td>"+formatter.format((total2-capitalFloat).toFixed(2))+"</td></tr>";
					}
				}
