...
<div class="container-form">
    <h3 class="title-form">Editar receita</h3>
    <hr style="margin-bottom: 20px; border: 0.5px solid #cecdcd;">

    <form action="<?= HOME_URL ?>/receita/editar?id=<?= $_GET['id'] ?>" method="POST">

        <label for="receitaRecebida" class="checkbox-container">
            <input type="checkbox" name="receitaRecebida" value="1" class="checkboxForm" <?= $dadosReceita[0]->status_trans == 'fechado' ? 'checked' : '' ?>>
            <span class="checkmark">Recebido</span> 
        </label>

        <label for="valor">Valor (R$):</label>
        <input type="text" class="money" name="valor" value="<?= $dadosReceita[0]->valor ?>">
        ...
        <select name="categoriaReceita">
            <?php if(count($listaCategorias) > 0): ?>
                <?php foreach($listaCategorias as $categoria): ?>
                    ...
                    <?php if($categoria->idCategoria == $dadosReceita[0]->id_categoria ): ?>
        ...
        <select name="ContaReceita">
             <?php if(count($listaContas) > 0): ?>
                <?php foreach($listaContas as $conta): ?>
                    ...
                    <?php if($conta->idConta == $dadosReceita[0]->id_conta ): ?>
